...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-vector-db-venv
    export POETRY_HOME := /tmp/poetry-vector-db-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-vector-db-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/vector-db-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-vector-db-cache
endif
